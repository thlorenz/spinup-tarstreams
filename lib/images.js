 ...
 *
 * @name spinupTarstreams::Images::buildStream
 * @function
 ...
 *
 * @name spinupTarstreams::Images::build
 * @function
 * @param {Stream} tarStream the tar stream that contains the project files and a `Dockerfile`
 * @param {string} image     then name under which to tag the created image
 * @param {function} cb      called back when image is created or with an error if one occurred
 ...
 * 
 * @name spinupTarstreams::Images::deserializeImageName
 * @function
 ...
 * Lists docker images.
 * 
 * @name spinupTarstreams::Images::list
 * @function
 ...
 * 
 * @name spinupTarstreams::Images::listGroup
 * @function
...
proto.removeGroup = function (group, cb) {
  ...
  self.listGroup(group, function (err, res) {
    ...
    var tasks = res.map(function (x) { 
      ...
      return function (cb_) {
        self.emit('debug', 'removing', inspect(x));
        img.remove(function (err) {
          if (err) return cb_(err);
          self.emit('info', 'removed', inspect({ name: x.RepoTags, id: x.Id }));
          cb_();
        })
      }
    });
